...
- [ ] T029 [US3] Install axe-core playwright package (`@axe-core/playwright`) in `frontend/package.json`
- [ ] T030 [US3] Create a11y audit script running axe-core against local dev server in `frontend/src/scripts/audit-a11y.ts`
- [ ] T031 [P] [US3] Fix semantic HTML issues (landmarks, single H1) in Expert dashboard views
- [ ] T032 [P] [US3] Fix semantic HTML issues in Student journey/dashboard views
- [ ] T033 [P] [US3] Fix semantic HTML issues in Parent dashboard views
- [ ] T034 [P] [US3] Fix semantic HTML issues in Public/Landing views
- [ ] T035 [US3] Remediate baseline contrast ratio and missing label critical findings based on audit report generated by scripts
...
- [ ] T036 [US4] Document semantic color scale and exemption comments mapping in `frontend/README.md` and/or quickstart docs.
...
- [ ] T037 Add `lint:design` and `audit:a11y` steps to CI pipeline in `.github/workflows/deploy.yml`
- [ ] T038 Validate full UI integrity by running `npm run build` & `npm run test`
- [ ] T039 Execute visual LTR/RTL cross-check manually and document passing state

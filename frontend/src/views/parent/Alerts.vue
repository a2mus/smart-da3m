...
<template>
  <div dir="rtl" class="min-h-screen bg-surface px-4 py-8">
    <div class="max-w-lg mx-auto">
      <h1 class="text-2xl font-black text-teal-700 mb-6">التنبيهات البيداغوجية</h1>
      <div class="space-y-3">
        ...
        >
          ...
          <div class="flex-1 min-w-0">
            <p class="font-medium text-sm">{{ a.messageAr }}</p>
            <p class="text-xs opacity-60 mt-1">{{ new Date(a.timestamp).toLocaleDateString('ar') }}</p>
          </div>
          <span v-if="!a.read" class="w-2 h-2 rounded-full bg-current shrink-0 mt-1.5" />
        </div>
      </div>
    </div>
  </div>

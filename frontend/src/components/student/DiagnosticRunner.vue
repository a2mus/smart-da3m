  ...
  >
    <div class="max-w-3xl mx-auto">
      ...
      >
        ...
        <div v-if="results.error_classification && results.error_classification !== 'NONE'" class="bg-surface-container rounded-2xl p-5 mb-6 text-start">
          <h3 class="font-bold text-on-surface mb-3">تحليل الأداء</h3>
        ...
        <!-- Finish Button -->
        <button
          class="w-full py-4 bg-primary text-on-primary font-bold rounded-xl
                 hover:bg-primary/90 active:scale-[0.98] transition-all min-h-[60px]"
      ...
      >
        ...
        >
          ...
          <p class="font-bold text-lg mb-1" :class="lastAnswerCorrect ? 'text-mint-600' : 'text-amber-600'">
            {{ lastAnswerCorrect ? 'إجابة صحيحة! أحسنت!' : 'ليس تماماً — لا تقلق!' }}
          </p>
          <p v-if="!lastAnswerCorrect && lastErrorClass" class="text-sm text-on-surface-variant">
            {{ errorClassLabels[lastErrorClass] || '' }}
          </p>
          <button
            v-if="!lastAnswerCorrect"
            class="mt-3 px-6 py-2 bg-primary text-on-primary rounded-xl font-bold
        ...
        <div class="mb-8">
          ...
          >
            <input
              ...
              type="text"
              :disabled="showFeedback"
              class="w-full p-5 rounded-2xl border-2 border-outline-variant
                     bg-surface-container-lowest text-on-surface
                     focus:border-primary focus:ring-4 focus:ring-primary/10
                     outline-none transition-all text-lg
                     font-['Inter','Tajawal'] text-center"
        ...
        <button
          ...
          class="w-full py-4 bg-primary text-on-primary font-bold rounded-xl
                 ...
                 flex justify-center items-center gap-2 min-h-[60px]"
          @click="submitAnswer"
        >
          <span
            v-if="isSubmitting"

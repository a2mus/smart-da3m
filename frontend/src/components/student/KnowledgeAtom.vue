...
<template>
  <div class="bg-surface rounded-2xl p-6 shadow-soft" data-testid="knowledge-atom">
    <!-- Header -->
    <div class="flex items-start gap-4 mb-6">
      ...
      <div class="flex-1">
        ...
        <h2 class="text-xl font-bold text-ink-800 mt-2">{{ atom.content.title }}</h2>
        <p class="text-ink-600 mt-1">{{ atom.content.description }}</p>
      </div>
    ...
    <div v-if="atom.remediation_type === 'SIMULATION'" class="mb-6 p-6 bg-ink-50 rounded-xl" @click="handleInteraction">
      ...
      <p class="text-center text-ink-600 mt-4">{{ t('remediation.dragToInteract') }}</p>
    </div>

    <!-- Mind Map Content -->
    <div v-if="atom.remediation_type === 'MIND_MAP'" class="mb-6 p-6 bg-ink-50 rounded-xl" @click="handleInteraction">
      <div class="flex flex-wrap justify-center gap-3">
        <div class="px-4 py-2 bg-teal-500 text-on-primary rounded-full text-sm">{{ t('remediation.mainConcept') }}</div>
        <div class="px-4 py-2 bg-teal-200 text-teal-800 rounded-full text-sm">{{ t('remediation.subConcept1') }}</div>
        <div class="px-4 py-2 bg-teal-200 text-teal-800 rounded-full text-sm">{{ t('remediation.subConcept2') }}</div>
        <div class="px-4 py-2 bg-ink-200 text-ink-700 rounded-full text-sm">{{ t('remediation.related') }}</div>
      </div>
    </div>
    ...
    <div class="mb-6">
      ...
      <div class="h-2 bg-ink-200 rounded-full overflow-hidden">
        <div class="h-full bg-teal-500 rounded-full transition-all duration-500" :style="{ width: `${progressPercent}%` }"></div>
      </div>
    </div>

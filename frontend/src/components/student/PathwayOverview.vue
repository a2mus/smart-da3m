...
<template>
  <div class="max-w-lg mx-auto">
    <h2 class="text-2xl font-black text-primary text-center mb-2">
      {{ competencyName }}
    </h2>
    <p class="text-on-surface-variant text-center mb-6 text-sm">
      مسار التقوية الخاص بك
    </p>
    ...
    <div class="space-y-3 mb-8">
      ...
      >
        ...
        >
          <span
            v-if="atom.status === 'completed'"
            class="material-symbols-outlined text-2xl"
          >check_circle</span>
          <span
            v-else-if="atom.status === 'locked'"
            class="material-symbols-outlined text-2xl"
          >lock</span>
          <span
            v-else
            class="material-symbols-outlined text-2xl"
          >{{ typeIcons[atom.type] || 'school' }}</span>
        </div>
